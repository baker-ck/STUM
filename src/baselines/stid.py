...
class STID(nn.Module):
    ...
    def __init__(self, **model_args):
        ...
        self.regression_layer = nn.Conv2d(
            in_channels=self.hidden_dim, out_channels=self.horizon, kernel_size=(1, 1), bias=True)
    def forward(self, history_data: torch.Tensor, label=None) -> torch.Tensor:
...
def get_engine_and_model(args):
    ...
    args.model = STID(**model_args)
    args.optimizer = torch.optim.Adam(args.model.parameters(), lr=args.lrate, weight_decay=args.wdecay)
    args.scheduler = torch.optim.lr_scheduler.StepLR(args.optimizer, step_size=args.step_size, gamma=args.gamma)
