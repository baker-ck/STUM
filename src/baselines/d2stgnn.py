...
from src.engines.d2stgnn_engine import *
from src.utils.graph_algo import *
...
def get_engine_and_model(args):
    ...
    args.cl_step = args.cl_epoch * args.dataloader['train_loader'].num_batch
    args.warm_step = args.warm_epoch * args.dataloader['train_loader'].num_batch
    args.model = D2STGNN(node_num=args.node_num,
                    input_dim=args.input_dim,
                    output_dim=args.output_dim,
                    model_args=vars(args)
                    )
    
    args.optimizer = torch.optim.Adam(args.model.parameters(), lr=args.lrate, weight_decay=args.wdecay, eps=1e-8)
    args.scheduler = torch.optim.lr_scheduler.MultiStepLR(args.optimizer, milestones=[1, 38, 46, 54, 62, 70, 80], gamma=0.5)
    
    #engine = D2STGNN_Engine(args=args, cl_step=cl_step, warm_step=warm_step, horizon = args.horizon)
